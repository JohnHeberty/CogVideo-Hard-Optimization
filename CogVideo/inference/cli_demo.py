...
def generate_video(
    ...
    # 3. Check VRAM and apply appropriate offload strategy
    log_vram_status("[Before Pipeline Load] ")
    
    is_sufficient, vram_msg = check_vram_availability(model_path)
    logging.info(f"VRAM Check: {vram_msg}")
    ...
    # Determine correct FPS if not explicitly provided or validate if provided
    final_fps = fps if fps is not None else get_correct_fps(model_path, num_frames)
    
    if fps is not None:
        # User specified FPS, validate it
        validate_fps_for_model(model_path, fps, num_frames)
    else:
        logging.info(f"Auto-detected FPS: {final_fps} for model {model_path}")
    
    export_to_video(video_generate, output_path, fps=final_fps)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fps", type=int, default=None, help="The frames per second for the generated video (auto-detected if not specified)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=f"Motion quality preset: {', '.join(list_presets())}. Overrides guidance_scale and num_inference_steps if specified.",
    )

    args = parser.parse_args()
    dtype = torch.float16 if args.dtype == "float16" else torch.bfloat16
    
    # Apply motion preset if specified (before other parameter overrides)
    if args.motion_preset:
        preset = get_preset(args.motion_preset)
        logging.info(f"Applying motion preset: {preset.name}")
        logging.info(f"  Description: {preset.description}")
        logging.info(f"  Guidance Scale: {preset.guidance_scale}")
        logging.info(f"  Inference Steps: {preset.num_inference_steps}")
        
        # Apply preset values (user CLI args take precedence if explicitly set)
        if args.guidance_scale == 6.0:  # Default value, apply preset
            args.guidance_scale = preset.guidance_scale
        if args.num_inference_steps == 50:  # Default value, apply preset
            args.num_inference_steps = preset.num_inference_steps
    
    # Auto-detect FPS if not provided
    if args.fps is None:
        args.fps = get_correct_fps(args.model_path, args.num_frames)
        logging.info(f"FPS auto-detected: {args.fps} for model {args.model_path}")
